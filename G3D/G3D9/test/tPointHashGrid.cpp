...
void testIterator(const PointHashGrid<Vector3>& grid, const Array<Vector3>& containedValues) {
    ...
    debugAssert(entries.size() == containedValues.size());
    for(int i = 0; i < entries.size(); ++i) {
        debugAssert(containedValues.contains(entries[i]));
        debugAssert(entries.contains(containedValues[i]));
    }
}
...
void testSphereIterator() {
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        for (PointHashGrid<Vector3>::SphereIterator iter = h.beginSphereIntersection(s); iter != end; ++iter) {
            Vector3 v = *iter;
            debugAssertM(s.contains(v), "SphereIterator returned a point that was not in the sphere");
        }
    }
}
...
void testPointHashGrid() {
    ...
    // Test size
    debugAssert(vec3Array.size() == grid.size());

    // Test conservativeBoxBounds
    AABox arrayBox(minCoords(vec3Array), maxCoords(vec3Array));
    debugAssert(arrayBox == grid.conservativeBoxBounds());

    // Test remove() and contains()	
    for(int i = 0; i < vec3Array.size(); ++i) {
        debugAssert(grid.contains(vec3Array[i]));
        debugAssert(grid.remove(vec3Array[i]) == true);
    }
    debugAssert(grid.size() == 0);
    debugAssert(!grid.contains(Vector3(-1.0, -1.0, -1.0)));
    debugAssert(grid.remove(Vector3(-1.0, -1.0, -1.0)) == false);
    ...
    for(PointHashGrid<Vector3>::CellIterator iter = grid.beginCells(); iter != end; ++iter) {
        debugAssert(iter->size() > 0);
        entriesFound += iter->size();
    }
    debugAssert(entriesFound == vec3Array.size());
...
void perfPointHashGrid() {
    ...
    */
    ...
    //debugAssert(tree.size() == hashGrid.size());
    printf("%d elements\n", numTestPts);
