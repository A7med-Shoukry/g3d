...
// MSVC seems to use the non-standard vsnprintf
...
std::string vformat(const char *fmt, va_list argPtr) {
    ...
    if (actualSize > bufSize) {
        ...
        if (actualSize < maxSize) {
            ...
            heapBuffer = (char*)System::memoryManager()->alloc(maxSize + 1);
            _vsnprintf(heapBuffer, maxSize, fmt, argPtr);
            heapBuffer[maxSize] = '\0';
        } else {
            heapBuffer = (char*)System::memoryManager()->alloc(actualSize);
            vsprintf(heapBuffer, fmt, argPtr);            
        }

        std::string formattedString(heapBuffer);
        System::memoryManager()->free(heapBuffer);
...
std::string vformat(const char* fmt, va_list argPtr) {
    ...
    if (numChars >= bufSize) {
      ...
      char* heapBuffer = (char*)System::memoryManager()->alloc((numChars + 1) * sizeof(char));
