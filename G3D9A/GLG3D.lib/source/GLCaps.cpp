...
void GLCaps::init() {
    ...
    if ((v != NULL) && (e == GL_NONE)) {
        #ifdef G3D_ANDROID
            // See http://www.khronos.org/opengles/sdk/docs/man/xhtml/glGetString.xml
            #define SL_VERSION_PREFIX_LEN (strlen("OpenGL ES GLSL ES "))
            sscanf(((const char*)glGetString(GL_SHADING_LANGUAGE_VERSION)) + SL_VERSION_PREFIX_LEN,
                   "%f", &m_glslVersion);
        #else
...
#       undef DECLARE_EXT
    ...
    // Some of these enums are unsupported in OpenGL ES.
    #ifndef G3D_ANDROID
        debugAssertGLOk();
    #endif
...
const std::string& GLCaps::glVersion() {
    ...
    #ifdef G3D_ANDROID
        #define VERSION_PREFIX_LEN (strlen("OpenGL ES "))
        static std::string _glVersion = ((char*)glGetString(GL_VERSION)) + VERSION_PREFIX_LEN;
    #else
        static std::string _glVersion = (char*)glGetString(GL_VERSION);
    #endif
    return _glVersion;
